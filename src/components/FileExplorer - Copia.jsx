                ...
                {items.map(item => (
                    <div key={item.id} className={`group relative p-2 rounded-lg border transition-all cursor-pointer select-none grid grid-cols-[auto_1fr_auto] gap-3 items-center ${item.id === activeDeckId ? 'bg-blue-50 border-blue-300 shadow-sm ring-1 ring-blue-200' : 'bg-white border-gray-100 hover:border-gray-300 hover:bg-gray-50'}`} onClick={() => onItemClick(item)}>
                        {isSelectionMode ? (
                            ...
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${item.type === 'folder' ? 'bg-yellow-100 text-yellow-600' : item.type === 'map' ? 'bg-teal-100 text-teal-600' : item.type === 'external_link' ? 'bg-gray-200 text-gray-600' : item.type === 'note' ? 'bg-indigo-100 text-indigo-600' : item.type === 'pdf' ? 'bg-red-100 text-red-600' : item.type === 'pdf_drive' ? 'bg-red-100 text-red-600' : item.type === 'video' ? 'bg-pink-100 text-pink-600' : item.type === 'image' ? 'bg-purple-100 text-purple-600' : item.type === 'edital' ? 'bg-green-100 text-green-600' : item.type === 'report' ? 'bg-orange-100 text-orange-600' : item.type === 'gdrive' ? 'bg-blue-100 text-blue-600' : item.type === 'shortcut' ? 'bg-gray-100 text-gray-600 border border-gray-300' : (item.id === activeDeckId ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500')}`}>
                                <Icon name={item.type === 'folder' ? 'folder' : item.type === 'map' ? 'share2' : item.type === 'external_link' ? 'externalLink' : item.type === 'note' ? 'fileText' : item.type === 'pdf' ? 'fileText': item.type === 'pdf_drive' ? 'fileText' : item.type === 'video' ? 'video' : item.type === 'image' ? 'image' : item.type === 'edital' ? 'list' : item.type === 'report' ? 'activity' : item.type === 'gdrive' ? 'hardDrive' : item.type === 'shortcut' ? 'link' : 'play'} size={14} className={item.id === activeDeckId ? "ml-0.5" : ""} />
                            </div>
                        )}
                        ...
                        {editingItemId !== item.id && !isSelectionMode && (
                            <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                <div className="flex items-center gap-1 mr-2">
                                    <button onClick={(e) => onOpenNewTab(e, item)} className="p-1.5 hover:bg-gray-100 text-gray-400 hover:text-blue-600 rounded" title="Abrir em Nova Aba (+)" ><Icon name="plusSquare" size={14} /> {/* ou use 'externalLink' */} </button>
                        ...
                        <div className="relative flex-1">
                            ...
                            {showNewMenu && (
                                <div className="absolute bottom-full left-0 w-full mb-2 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden flex flex-col animate-in slide-in-from-bottom-2 fade-in duration-200 z-50">
                                    ...
                                    <button onClick={() => { onCreatePDFDrive(); setShowNewMenu(false); }} className="text-left px-4 py-3 hover:bg-gray-50 text-sm text-gray-700 flex items-center gap-2"><Icon name="fileText" size={16} className="text-red-600"/> PDF 2.0 (Drive)</button>
                                    <button onClick={() => { onCreateMap(); setShowNewMenu(false); }} className="text-left px-4 py-3 hover:bg-gray-50 text-sm text-gray-700 flex items-center gap-2 border-b border-gray-50"><Icon name="share2" size={16} className="text-teal-500"/> Mapa Mental </button>
                                    <button onClick={() => { onCreateExternalLink(); setShowNewMenu(false); }} className="text-left px-4 py-3 hover:bg-gray-50 text-sm text-gray-700 flex items-center gap-2 border-b border-gray-50"><Icon name="externalLink" size={16} className="text-gray-600"/> Link Externo </button>
                                </div>
                            )}
                        </div>
