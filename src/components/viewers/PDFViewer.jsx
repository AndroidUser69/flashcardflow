...
// --- IMPORTS NECESSÁRIOS PARA O CARREGAMENTO INDEPENDENTE ---
import { dbHelper } from '../../utils/dbHelper';
import { GOOGLE_DRIVE_API_KEY } from '../../utils/constants';
...
// --- COMPONENTE DE PÁGINA VIRTUAL (MANTIDO IGUAL) ---
...
// Removemos pdfUrl e pdfMissing das props, pois agora ele gerencia isso internamente
const PDFViewer = ({ currentItem, onUpdateHighlights, onSaveHighlights }) => {
    
    // --- NOVO ESTADO INTERNO PARA GERENCIAR O ARQUIVO ---
    ...
    // --- 1. LÓGICA DE CARREGAMENTO DO ARQUIVO (A MÁGICA DA PERSISTÊNCIA) ---
    useEffect(() => {
        let active = true;
        let objectUrl = null;
        ...
        const loadFile = async () => {
            ...
            try {
                if (currentItem.type === 'pdf') {
                    // --- 1. PDF LOCAL (IndexedDB) ---
                    const blob = await dbHelper.getFile(currentItem.id);
                    if (blob && active) {
                        objectUrl = URL.createObjectURL(blob);
                        setInternalPdfUrl(objectUrl);
                    } else if (active) {
                        setIsMissing(true);
                    }
                } else if (currentItem.type === 'pdf_drive') {
                    // --- 2. PDF DRIVE (API OFICIAL V3) ---
                    const driveId = currentItem.content;
                    
                    // URL Oficial da API (Funciona se o site estiver autorizado no Console)
                    const apiUrl = `https://www.googleapis.com/drive/v3/files/${driveId}?alt=media&key=${GOOGLE_DRIVE_API_KEY}`;
                    
                    const res = await fetch(apiUrl);
                    
                    if (!res.ok) {
                        // Se der erro 403/401, geralmente é erro de CORS ou Permissão
                        console.error(`Erro Drive API: ${res.status} ${res.statusText}`);
                        throw new Error('Erro ao baixar do Drive. Verifique as permissões da Chave de API.');
                    }
                ...
                if (active) {
                    ...
                    if (err.message.includes('Failed to fetch')) {
                        console.warn("DICA: Verifique se 'https://androiduser69.github.io' está adicionado na sua Chave de API no Google Cloud.");
                    }
                }
            } finally {
        ...
        // Cleanup: Só limpa a URL quando a aba for FECHADA (componente desmontado)
        // Isso garante que enquanto estiver oculto (display: none), a URL vive.
        return () => {
            active = false;
            if (objectUrl) URL.revokeObjectURL(objectUrl);
        };
    }, [currentItem.id, currentItem.type, currentItem.content]);

    // Função de Restauração Local
    const handleRestore = async (e) => {
        const file = e.target.files[0];
        if (file) {
            await dbHelper.saveFile(currentItem.id, file);
    ...
    const handleSaveJson = () => {
        try {
            ...
            if (onSaveHighlights) {
                ...
                alert("Salvo via método alternativo.");
            } else {
    ...
    return (
        <div className="flex flex-col h-full bg-gray-100 overflow-hidden w-full relative">
            ...
            <div className="flex-1 overflow-hidden relative">
                ...
                >
                    {/* --- USANDO O ESTADO INTERNO (internalPdfUrl) --- */}
                    {!internalPdfUrl ? (
                        <div className="flex flex-col items-center justify-center h-full text-gray-500 gap-3">
                            {isMissing ? (
                                <>
                                    <Icon name="alertCircle" size={48} className="text-red-400 mb-4"/>
                                    <p className="text-lg">Arquivo não encontrado.</p>
                                    {currentItem.type === 'pdf' && (
                                        <button onClick={() => document.getElementById(`restore-pdf-${currentItem.id}`).click()} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg shadow-md font-medium text-sm mt-2">
                                            Restaurar Arquivo
                                        </button>
                                    )}
                                    <input id={`restore-pdf-${currentItem.id}`} type="file" accept=".pdf" className="hidden" onChange={handleRestore} />
                                </>
                            ) : (
                                <><div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div><p>Carregando PDF...</p></>
                            )}
                        </div>
                    ) : (
                        <div 
                        ...
                        > 
                            {/* Passa internalPdfUrl para o Document */}
                            <Document file={internalPdfUrl} key={currentItem.id} onLoadSuccess={onDocumentLoadSuccess} className="outline-none">
